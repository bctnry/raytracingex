...
for a in -11..<11:
  for b in -11..<11:
    ...
    if (center - Vec3(x: 4, y: 0.2, z: -1.0)).length() > 0.9 and (center - Vec3(x: -4, y: 0.2, z: -1.0)).length() > 0.9:
      if chooseMat < 0.8:
...
world.add(Sphere(center: Vec3(x: -4.0, y: 1.0, z: 1.0), radius: 1.0, mat: material2))
let material3 = makeMetal(makeColor(0.7, 0.6, 0.5), 0.0)
world.add(Sphere(center: Vec3(x: 4.0, y: 1.0, z: -1.0), radius: 1.0, mat: material3))
